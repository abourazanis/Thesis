...
public class EpubsDatabase {
	...
	public static ContentProviderOperation buildEpubOp(BookLink epub,
		...
		if (!StringUtil.isNotBlank(epubId)) {
			...
			return ContentProviderOperation
					.newUpdate(Epubs.buildEpubUri(epubId)).withValues(values)
					.build();
		}
	}
	...
	private static ContentValues putCommonShowValues(BookLink epub,
		...
		Resource cover = meta.getCoverImage();
		List<String> subjects = meta.getSubjects();
		List<String> descriptions = meta.getDescriptions();

		values.put(Epubs.TITLE, meta.getFirstTitle());
		if (!subjects.isEmpty())
			values.put(Epubs.SUBJECT, subjects.get(0));
		values.put(Epubs.FILENAME, epub.getId());
		if (cover != null)
			values.put(Epubs.COVERDATA, cover.getData());

		values.put(Epubs.LANGUAGE, meta.getLanguage());
		if (!descriptions.isEmpty())
			values.put(Epubs.DESCRIPTION, descriptions.get(0));
	...
	public static boolean isAuthorExists(Author author, Context context) {
		Cursor search = context.getContentResolver().query(Authors.CONTENT_URI,
				new String[] { Authors._ID },
	...
	public static void addEpub(BookLink epub, Context context) {
		...
		try {
			...
			final ArrayList<ContentProviderOperation> batch_b = Lists
					.newArrayList();
	...
	public static String getEpubLocation(String epubId, Context context) {
		...
		if (search.moveToFirst()) {
			location = search.getString(search
	...
	public static void onRenewFTSTable(Context context) {
		context.getContentResolver().query(
				EpubSearch.CONTENT_URI_RENEWFTSTABLE, null, null, null, null);
	}
}
